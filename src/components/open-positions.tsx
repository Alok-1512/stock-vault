...
import type { OpenPosition, CurrentPrices } from '@/lib/types';
import { formatCurrencyFull, formatPercent, formatDays, plColor, plBg } from '@/lib/format';
import { Input } from '@/components/ui/input';
import { ArrowUpRight, ArrowDownRight, Pencil, Check } from 'lucide-react';
...
export function OpenPositions({ positions, onUpdatePrice }: Props) {
  ...
  useEffect(() => { setNow(Date.now()); }, [positions]);

  if (positions.length === 0) {
    return (
      <div className="rounded-xl border border-border bg-card p-12 text-center">
        <p className="text-muted-foreground text-sm">No open positions. Log a buy trade to get started.</p>
      </div>
    );
  }

  // Find best and worst
  const best = positions.reduce((a, b) => (a.unrealizedPLPercent > b.unrealizedPLPercent ? a : b));
  const worst = positions.reduce((a, b) => (a.unrealizedPLPercent < b.unrealizedPLPercent ? a : b));
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              ...
              <th className="text-right px-4 py-3 text-xs font-semibold text-muted-foreground">Shares</th>
              <th className="text-right px-4 py-3 text-xs font-semibold text-muted-foreground">Avg Price</th>
              <th className="text-right px-4 py-3 text-xs font-semibold text-muted-foreground">Invested</th>
              <th className="text-right px-4 py-3 text-xs font-semibold text-muted-foreground">Current Price</th>
          ...
          <tbody>
            {positions.map((pos) => {
              const isBest = pos.ticker === best.ticker && pos.unrealizedPLPercent > 0;
              const isWorst = pos.ticker === worst.ticker && pos.unrealizedPLPercent < 0;
              const daysSinceFirst = Math.floor(
                (now - new Date(pos.earliestBuyDate).getTime()) / (1000 * 60 * 60 * 24)
              );
              ...
              return (
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground font-mono">{pos.ticker}</p>
                      </div>
                      {isBest && (
                  ...
                  <td className="px-4 py-3 text-right font-mono">{pos.totalShares}</td>
                    ...
                    {editingTicker === pos.ticker ? (
                      <div className="flex items-center gap-1 justify-end">
                        <Input
                          ...
                          onKeyDown={(e) => e.key === 'Enter' && savePrice(pos.ticker)}
                          autoFocus
                        />
                        <button
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <span className={`font-mono font-semibold ${plColor(pos.unrealizedPL)}`}>
                        {formatCurrencyFull(pos.unrealizedPL)}
                      </span>
                    </div>
                    <p className={`text-xs font-mono ${plColor(pos.unrealizedPLPercent)}`}>
